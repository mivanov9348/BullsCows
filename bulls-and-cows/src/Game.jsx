...
const playSound = (soundName) => {
  ...
  audio.play().catch(e => console.log('Звукът е блокиран от браузъра (изисква се интеракция):', e));
};
...
export default function Game() {
  ...
  useEffect(() => {
    if (heartbeatAudio.current) {
      if (lives <= 3 && lives > 0 && !isLevelCleared && !isGameOver) {
        ...
        heartbeatAudio.current.play().catch(e => console.log('Сърцебиенето чака клик', e));
      } else {
  ...
  // --- НОВА ФУНКЦИЯ ЗА ЗАПАЗВАНЕ ВЪВ FIREBASE ---
  ...
  const useJoker = () => {
    if (lives <= GAME_SETTINGS.JOKER_COST_LIVES) {
      showError(`Трябват ти повече от ${GAME_SETTINGS.JOKER_COST_LIVES} живота за Жокер!`);
      return;
    }

    const secretArray = secret.split('');
    const guessedDigits = new Set(history.map(h => h.guess).join('').split(''));
    const unrevealedDigits = secretArray.filter(digit => !guessedDigits.has(digit));

    let revealedDigit;
    if (unrevealedDigits.length > 0) {
      const randomIndex = Math.floor(Math.random() * unrevealedDigits.length);
      revealedDigit = unrevealedDigits[randomIndex];
    } else {
      const randomIndex = Math.floor(Math.random() * secretArray.length);
      revealedDigit = secretArray[randomIndex];
    }

    setLives(prevLives => prevLives - GAME_SETTINGS.JOKER_COST_LIVES);
    playSound('joker');
    showInfo(`💡 Жокер: Цифрата ${revealedDigit} участва в числото! (-${GAME_SETTINGS.JOKER_COST_LIVES} ❤️)`);
  };
  ...
  // --- ПРОМЕНЕНО НА ASYNC ЗАРАДИ FIREBASE ---
  ...
  return (
    <>
      ...
      <div className="flex flex-col lg:flex-row gap-6 w-full max-w-4xl justify-center items-start z-10 relative">
        <div className={`bg-white/95 backdrop-blur-sm rounded-3xl w-full lg:max-w-md p-8 border relative overflow-hidden transition-all duration-500
          ${isDangerZone ? 'border-red-500 shadow-[0_0_50px_rgba(239,68,68,0.4)]' : 'border-white/20 shadow-2xl'}
        `}>
          ...
          <div className="flex justify-between items-center bg-gray-50 p-4 rounded-2xl mb-6 border border-gray-100 shadow-inner mt-4 relative z-10">
            ...
            <div className="text-center">
              ...
              <span className={`text-2xl font-black ${isDangerZone ? 'text-red-600 animate-bounce' : 'text-pink-500'}`}>
                {lives} ❤️
              </span>
            </div>
            <div className="text-center">
            ...
            <form onSubmit={handleGuess} className="flex flex-col gap-3 mb-6 relative z-10">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="bg-gradient-to-r flex-1 from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 disabled:from-gray-300 disabled:to-gray-400 disabled:text-gray-500 text-white px-6 py-3 rounded-xl font-bold transition-all shadow-md hover:shadow-lg disabled:shadow-none"
                >
                  Опитай
                </button>
                <button
                  type="button"
                  onClick={useJoker}
                  disabled={lives <= GAME_SETTINGS.JOKER_COST_LIVES}
                  className="bg-blue-100 hover:bg-blue-200 text-blue-700 disabled:bg-gray-100 disabled:text-gray-400 border border-blue-200 px-4 py-3 rounded-xl text-sm font-bold transition-all shadow-sm flex items-center justify-center gap-1 group relative whitespace-nowrap"
                >
                  💡 Жокер
                  <span className="hidden group-hover:block absolute -top-10 bg-gray-800 text-white text-xs px-3 py-1 rounded-lg w-max shadow-lg z-10">
                    Струва {GAME_SETTINGS.JOKER_COST_LIVES} ❤️
                  </span>
                </button>
              </div>
            </form>
