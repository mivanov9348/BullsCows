...
export default function Leaderboard({ onBack }) {
  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      try {
        const q = query(collection(db, "wordle_leaderboard"), orderBy("score", "desc"), limit(10));
  ...
  return (
    <div className="bg-white/10 backdrop-blur-md rounded-3xl shadow-2xl w-full max-w-md p-8 border border-white/20 text-white mx-4 z-20 relative">
      ...
      <div className="text-center mb-8">
        <h2 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-emerald-500 mb-2">Глобална Класация 🌍</h2>
        <p className="text-sm font-medium text-gray-300">Думички (Топ 10)</p>
      </div>

      <div className="space-y-3 custom-scrollbar overflow-y-auto max-h-80 pr-2 min-h-[150px] relative">
        {loading && <div className="absolute inset-0 flex items-center justify-center bg-gray-900/50 backdrop-blur-sm z-10 rounded-xl"><div className="animate-spin rounded-full h-8 w-8 border-b-4 border-green-500"></div></div>}
        {!loading && scores.length > 0 ? (
          scores.map((score, index) => (
            <div key={index} className={`flex items-center justify-between p-3 rounded-xl border ${index === 0 ? 'bg-yellow-500/20 border-yellow-400/50' : index === 1 ? 'bg-gray-400/20 border-gray-300/50' : index === 2 ? 'bg-orange-500/20 border-orange-400/50' : 'bg-white/5 border-white/10'}`}>
              ...
              <div className="font-black text-green-400 text-lg">{score.score} <span className="text-xs text-gray-400">т.</span></div>
            </div>
          ))
